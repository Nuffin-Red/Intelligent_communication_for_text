...
def main(args):
    ...
    for fn in tqdm(os.listdir(args.input_data_dir)):
        ...
        text_paths.append(os.path.join(args.input_data_dir, fn))
        # process_sentences = process(os.path.join(args.input_data_dir, fn))
        # sentences += process_sentences
    process_sentences = multi_process(process, text_paths, 1)
    for ps in process_sentences:
        sentences += ps
    #sentences是所有句子的的列表
    ...
    for seq in tqdm(sentences):
        ...
        token = [token_to_idx[word] for word in words]
