...
def subsequent_mask(size):
    ...
    return torch.from_numpy(subsequent_mask)#numpy转tensor
...
def create_masks(src, trg, padding_idx):
    ...
    #decoder 的 self-attention，同时需要padding mask 和 sequence mask 作为 attn_mask，具体实现就是两个mask相加作为attn_mask。
...
def train_step(model, src, trg, n_var, pad, opt, criterion, channel, mi_net=None):
    # src=trg=sents,n_var=noise,pad=pad_index,opt=optimizer,channel=Rayleigh
    model.train()
    #训练模式使用.train()

    trg_inp = trg[:, :-1] # batch_size, maxlen-1
    trg_real = trg[:, 1:]

    channels = Channels()
    opt.zero_grad()
    
    src_mask, look_ahead_mask = create_masks(src, trg_inp, pad)  # create mask for pad index
    #src_mask(128,1,30)
    #look_ahead_mask(128,30,30)
    ...
    channel_dec_output = model.channel_decoder1(Rx_sig)
    dec_output = model.decoder1(trg_inp, channel_dec_output, look_ahead_mask, src_mask)
    #
    pred = model.dense1(dec_output)
    ntokens = pred.size(-1)
...
def greedy_decode(model, src, n_var, max_len, padding_idx, start_symbol, channel):
    ...
    outputs = torch.ones(src.size(0), 1).fill_(start_symbol).type_as(src.data)#在开头添加开始标签
