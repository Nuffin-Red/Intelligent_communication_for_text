...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, look_ahead_mask, trg_padding_mask):
        ...
        src_output = self.src_mha(x, memory, memory, trg_padding_mask) # q：x上一层的输出；k, v，memory来自编码器的输出
...
class Encoder(nn.Module):
    ...
    def forward(self, x, src_mask):
        ...
        for enc_layer in self.enc_layers:
            x = enc_layer(x, src_mask)#def forward(self, x, mask):
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, look_ahead_mask, trg_padding_mask):
        ...
        for dec_layer in self.dec_layers:
            x = dec_layer(x, memory, look_ahead_mask, trg_padding_mask)#def forward(self, x, memory, look_ahead_mask, trg_padding_mask):
...
class DeepSC(nn.Module):
    def __init__(self, num_layers, src_vocab_size, trg_vocab_size, src_max_len, 
        ...
        # d_model=dimension,dff=512(for position)
        super(DeepSC, self).__init__()

        # word embedding + position embedding -> 4layers multi-head attention + position wise forward
        # <multi-head attention> use three linear layer train self-key, self-query, self-value and then use
        # change their dimension to batch_size, 1, num_heads, heads_dimension and apply to attention function
        # <position wise forward> use linear layer train dimension from d_model to dff and then from dff to d_model
        ...
        # take x and memory to multi-head attention layers
