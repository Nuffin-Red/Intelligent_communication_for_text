...
parser.add_argument('--bert-config-path', default='bert/caesd_L-12_H-768_A-12/bert_config.json', type = str)
...
def performance(args, SNR, net):
    ...
    #批训练，把数据变成一小批一小批数据进行训练。DataLoader就是用来包装所使用的数据，每次抛出一批数据，# 从数据库中每次抽出batch size个样本test_iterator中有64个句子
