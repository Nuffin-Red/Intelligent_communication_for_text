...
def train(epoch, args, net, mi_net=None):
    ...
    for sents in pbar:
        sents = sents.to(device) # batch_size, maxlen
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        if avg_acc < record_acc:
            ...
            record_acc = avg_acc
    record_loss = []
#optimizer = torch.optim.Adam(model.parameters(),),定义优化方法
#criterion = nn.CrossEntropyLoss(reduction = 'none')定义训练标准：
#for epoch in epochs:
#   for batch in batchs:
#      optimizer.zero_grad()先将梯度归零
#      out = model(input)
#      loss = lossfuction(out,input)
